...
Description:支持7种方式返回图片数据--https://blog.qcair.cc/cnapi.html
Version:1.0.16
ForPHP:5.6+
...
//------------设置区域开始------------
$APIname = "QCAPI-CN-Cherry";
//此处填写API名称
$domainPrefix = 'acg.qcair.cc';
//API域名设置
$Block_IP = "off";
//开启恶意IP拦截功能则填写on,反之填写off,如果你的网站
$base64Times = "10";
//由于Base64转码会消耗服务器资源，此处填写每分钟限制Base64数据输出次数，例如填写10则单用户每分钟只可调用返回数据10次，填写0或负数则不限制每分钟调用次数
$img_Array = glob("images/cherry/*.{gif,jpg,png,jpeg,JPG,PNG,GIF,JPEG}",GLOB_BRACE); 
...
$localimageUrl='http://127.0.0.1:2333/'.$img_Array[$imageFile];
...
//Base64转码格式
//获取图像
$base64ImgCode = get_curl($localimageUrl);
...
switch ($returnType) {
	case 'url':
		echo $imageUrl;
		echo "<br>";
		echo "200-OK";//$_SERVER['SERVER_NAME']
		echo "<br>";
	...
	case 'img':
		...
		$img = get_curl($localimageUrl, true);
...
$Count = file_get_contents("./Main/countCherry.txt");
//读取数据文件
if (!$_SESSION['#']) {
	$_SESSION['#'] = true;
	$Count++;
	//刷新一次+1
	$ApiTimes = fopen("./Main/countCherry.txt", "w");
